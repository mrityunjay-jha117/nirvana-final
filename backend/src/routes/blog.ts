...
// paginated bulk route
blogRouter.get("/paginated_bulk", async (c) => {
  const prisma = new PrismaClient({
    datasourceUrl: c.env.DATABASE_URL,
  }).$extends(withAccelerate());

  // Get pagination parameters from query, with defaults
  const page = Number(c.req.query("page") || 1); // current page
  const limit = Number(c.req.query("limit") || 6); // items per page

  const skip = (page - 1) * limit;

  try {
    // Fetch paginated blogs with author and location
    const blogs = await prisma.blog.findMany({
      skip,
      take: limit,
      include: {
    ...
    // Get total blog count for frontend to calculate total pages
...
blogRouter.get("/stats", async (c) => {
  ...
  try {
    ...
    const totalLikes = (stats._sum as { likes: number | null })?.likes ?? 0;
    const blogCount = (stats._count as { id: number })?.id ?? 0;
...
// Like buffer: blogId -> like count
const likeBuffer = new Map<string, number>();
...
blogRouter.post("/like", async (c) => {
  ...
  if (!blogId) {
    c.status(400);
    return c.json({ message: "Missing blogId in request" });
  }

  // Update in-memory buffer
  const currentCount = likeBuffer.get(blogId) || 0;
  likeBuffer.set(blogId, currentCount + 1);

  // If buffer is full, flush to DB
  if (likeBuffer.get(blogId)! >= BUFFER_LIMIT) {
    try {
      await prisma.blog.update({
        where: { id: blogId },
        data: {
          likes: { increment: BUFFER_LIMIT },
        },
      });
      likeBuffer.set(blogId, 0); // Reset buffer
    } catch (error) {
      c.status(500);
      return c.json({
  ...
  return c.json({ message: "Like registered (buffered)" });
});
